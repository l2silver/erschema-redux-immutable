...
export default {
  create(modelGenerator: $modelGenerator, location: $location = []) {
  ...
  get(modelGenerator: $modelGenerator, location: $location = []) {
    return function (state: Object, {payload, error}: Object) {
      if (error) {
        return state
      }
  ...
  index(modelGenerator: $modelGenerator, location: $location = []) {
    return function (state: Object, {payload, error}: Object) {
      if (error) {
        return state
      }
